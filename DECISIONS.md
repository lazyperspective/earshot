...
## Post-submission — Long recordings

- **Zoom is pixels-per-second on a log scale, 0 = fit.** Files longer than five minutes open showing two minutes instead of squeezing the whole episode into one screen; presets (Fit · 10m · 2m · 30s · 5s), a log slider with "mm:ss in view", ⌘/ctrl-wheel or trackpad pinch zooms around the cursor, a plain wheel scrolls the timeline, `+`/`−`/`0` keys, a thin scrollbar, and a minimap overview with a draggable viewport window. `set_view` exposes the same controls to the agent and `get_status` reports the current view.
- **Envelope-based analysis.** One pass over the channel data produces a per-10 ms mean-square / peak / clipped-count envelope (~3 MB for two hours) computed in 10 s slices with yields; every analysis tool reads the envelope, so nothing copies the audio and the UI never freezes. The mono helpers remain as thin wrappers for tests.

/**
 * Vercel function: proxies raw audio bytes to OpenAI's transcription endpoint (whisper-1, word timestamps).
 * The API key never leaves the server. `transcribeCore` is also imported by the Vite dev middleware so
 * `npm run dev` works locally with OPENAI_API_KEY in .env. Kept in one file: Vercel's ESM runtime needs
 * explicit extensions for relative imports, so a helper module would not resolve at runtime.
 */

...
export function fxForToolResult(tool: string, args: Any, result: unknown): void {
  ...
  switch (tool) {
    ...
    case 'detect_silences': { const rs = rangesOf(r.silences); emitFx({ type: 'ranges', kind: 'silence', ranges: rs, label: `${r.count ?? ''} silences` }); useStore.getState().setFinding('silence', rs, `${r.count ?? rs.length} silences`); break; }
    case 'detect_clipping': { const rs = rangesOf(r.regions); emitFx({ type: 'ranges', kind: 'clip', ranges: rs, label: 'clipping' }); useStore.getState().setFinding('clip', rs, rs.length ? `${rs.length} clipped` : 'no clipping'); break; }
    case 'find_filler_words': { const rs = rangesOf(r.fillers); emitFx({ type: 'ranges', kind: 'filler', ranges: rs, label: `${r.count ?? ''} fillers` }); emitFx({ type: 'words', kind: 'filler', ids: idsOf(r.fillers) }); useStore.getState().setFinding('filler', rs, `${r.count ?? rs.length} fillers`); useStore.getState().setFindingWords(idsOf(r.fillers), 'filler'); break; }
    case 'find_in_transcript': { const rs = rangesOf(r.matches); emitFx({ type: 'ranges', kind: 'match', ranges: rs, label: `${r.count ?? ''} matches` }); emitFx({ type: 'words', kind: 'match', ids: idsOf(r.matches) }); useStore.getState().setFinding('match', rs, `${r.count ?? rs.length} matches “${String(r.query ?? '').slice(0, 24)}”`); useStore.getState().setFindingWords(idsOf(r.matches), 'match'); break; }
    case 'compare_speakers': { const q = rangesOf((r.segments ?? []).filter((s: Any) => s.quiet)); const l = rangesOf((r.segments ?? []).filter((s: Any) => s.loud)); emitFx({ type: 'ranges', kind: 'quiet', ranges: q, label: 'quiet' }); emitFx({ type: 'ranges', kind: 'loud', ranges: l, label: 'loud' }); useStore.getState().setFinding('quiet', q, `${q.length} quiet spans`); if (l.length) useStore.getState().setFinding('loud', l, `${l.length} loud spans`); break; }
    case 'get_loudness_profile': { const pts = Array.isArray(r.points) ? (r.points as Any[]).map((p) => ({ t: Number(p.t), db: Number(p.rms_db) })) : []; if (pts.length) { emitFx({ type: 'curve', points: pts, label: `loudness · ${r.integrated_db} dBFS` }); useStore.getState().setFindingCurve(pts, `loudness · ${r.integrated_db} dBFS`); } break; }
    ...
    case 'suggest_cuts': { const c = rangesOf(r.candidates); const p = rangesOf(r.pause_candidates); emitFx({ type: 'ranges', kind: 'filler', ranges: c, label: `${r.candidate_count ?? ''} candidates` }); emitFx({ type: 'ranges', kind: 'silence', ranges: p }); emitFx({ type: 'words', kind: 'filler', ids: idsOf(r.candidates) }); useStore.getState().setFinding('filler', c, `${c.length} cut candidates`); if (p.length) useStore.getState().setFinding('silence', p, `${p.length} long pauses`); useStore.getState().setFindingWords(idsOf(r.candidates), 'filler'); break; }
